...
3.  Määrasin joonlauaga vedru alumise otsa asendi (ilma raskuseta). Panin selle mõõtmistulemuse (vedru tasakaaluasend $x_{balance}$) kirja.
    ...
    $x_{balance} := \mathrm{1.04 \times 10^{-1} \times m}$
...
```{r label = "determination-of-spring-stiffness"}
...
print_table(determination_of_spring_stiffness, caption = "Vedru jälkuse määramine", digits = 6) %>% kable_styling(    bootstrap_options = c("striped", "bordered", "condensed", "responsive", "hover"),
...
2. Kinnitasin vedru statiivi külge nii, et see sai vabalt üles-alla liikuda. Riputasin vedru otsa raskused, sobivaim oli kasutada vedru otsas olevaid raskusi kogumassiga 400 kuni 600 grammi (joonis \@ref(fig:harmonic-oscillation)).
...
4. Mõõtsin joolauaga vedrupendli alumise otsa kauguse liikumissensorist, kui pendel ei võnkunud (tasakaaluasend $x_{tas}'$) või võtsin liikumissensori lugemi, kui pendel ei liikunud, vajutades korraks nuppu *Start*, ja seejärel katkestasin katse vajutades nupule *Stop*.
